        ...
        if response.status_code != 404:
            try:
                ...
                if result['name'] not in [r.name for r in user.roles]:
                    ...
                    await self.bot.say("Greetings from " + result['name'] + " :flag_"+ result['alpha2Code'].lower() +": by " + user.mention)
                else:
    ...
    async def removecountry(self, ctx, country: str):
        ...
        if response.status_code != 404:
            ...
            if result['name'] in [r.name for r in user.roles]:
                ...
                await self.bot.say("The boys and girls from " + result['name'] + " will miss you " + user.mention + "! :(")
            else:
